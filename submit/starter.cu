...
__global__ void CalculateInvertedGrayScale(long int *red, long int *green, long int *blue, long int *T1, int m, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx<(m*n)) {
        int row = idx/n;
        int col = idx%n;
        long int gray=(red[idx] + green[idx] + blue[idx]) /3;
        T1[(m-1-row)*n+col]=gray;
    }
}
__global__ void CalculateThomasTransformation(long int *red, long int *green, long int *blue, long int *T2, int m, int n) {
    int idx=blockIdx.x * blockDim.x + threadIdx.x;
    if (idx<(m*n)) {
        T2[idx]=floor(0.5*red[idx]) + floor(sqrt((double)green[idx])) + blue[idx];
    }
}
...
{   
    ...
    // const char *inputFileName = "/content/drive/MyDrive/Colab Notebooks/Assignment1/input/3";
