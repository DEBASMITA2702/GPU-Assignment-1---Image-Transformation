...
__global__ void CalculateInvertedGrayScale(long int *red, long int *green, long int *blue, long int *T1, int a, int b) {
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if(index < (a*b))
    {
        long int invertedGray=(red[index] + green[index] + blue[index])/3;
        T1[(a-1-(index/b))*b+(index%b)]=invertedGray;
    }
  }
  __global__ void CalculateThomasTransformation(long int *red, long int *green, long int *blue, long int *T2, int a, int b) {
    int index = blockIdx.x*blockDim.x+threadIdx.x;
    if (index < (a*b)) 
    {
        long int thomas=floor(0.5*red[index]) + floor(sqrt((double)green[index])) + blue[index];
        T2[index]=thomas;
    }
  }
